...
# Variabel input barang terjual (0–100)
permintaan      = ctrl.Antecedent(np.arange(0, 301, 1), 'permintaan')
# Variabel input permintaan (0–300)
harga_item      = ctrl.Antecedent(np.arange(0, 100001, 1), 'harga_item')
# Variabel input harga per item (0–100.000)
profit          = ctrl.Antecedent(np.arange(0, 4000001, 1), 'profit')
# Variabel input profit (0–4.000.000)
stok_makanan    = ctrl.Consequent(np.arange(0, 1001, 1), 'stok_makanan')
# Variabel output stok makanan (0–1000)
 

# 2. Membership Functions (Menggunakan fungsi segitiga/trimf sebagai standar atau trapesium/trapmf jika diperlukan)
# Barang Terjual [0 - 100] dengan 3 himpunan fuzzy: rendah, sedang, tinggi
...
# Permintaan [0 - 300] dengan 3 himpunan fuzzy: rendah, sedang, tinggi
...
# Harga per Item [0 - 100.000] dengan 3 himpunan fuzzy: murah, sedang, mahal
...
# Profit [0 - 4.000.000] dengan 3 himpunan fuzzy: rendah, sedang, tinggi
...
# Stok Makanan (Output) [0 - 1000] dengan 2 himpunan fuzzy: sedang, banyak
...
# Jika Barang Terjual tinggi dan Permintaan tinggi dan Harga per Item murah dan Profit tinggi maka Stok Makanan banyak
rule2 = ctrl.Rule(barang_terjual['tinggi'] & permintaan['tinggi'] & harga_item['murah'] & profit['sedang'], stok_makanan['sedang'])
# Jika Barang Terjual tinggi dan Permintaan tinggi dan Harga per Item murah dan Profit sedang maka Stok Makanan sedang
rule3 = ctrl.Rule(barang_terjual['tinggi'] & permintaan['sedang'] & harga_item['murah'] & profit['sedang'], stok_makanan['sedang'])
# Jika Barang Terjual tinggi dan Permintaan sedang dan Harga per Item murah dan Profit sedang maka Stok Makanan sedang
rule4 = ctrl.Rule(barang_terjual['sedang'] & permintaan['tinggi'] & harga_item['murah'] & profit['sedang'], stok_makanan['sedang'])
# Jika Barang Terjual sedang dan Permintaan tinggi dan Harga per Item murah dan Profit sedang maka Stok Makanan sedang
rule5 = ctrl.Rule(barang_terjual['sedang'] & permintaan['tinggi'] & harga_item['murah'] & profit['tinggi'], stok_makanan['banyak'])
# Jika Barang Terjual sedang dan Permintaan tinggi dan Harga per Item murah dan Profit tinggi maka Stok Makanan banyak
rule6 = ctrl.Rule(barang_terjual['rendah'] & permintaan['rendah'] & harga_item['sedang'] & profit['sedang'], stok_makanan['sedang'])
#  Jika Barang Terjual rendah dan Permintaan rendah dan Harga per Item sedang dan Profit sedang maka Stok Makanan sedang


# 4. Sistem Kontrol dan Simulasi
stok_ctrl = ctrl.ControlSystem([rule1, rule2, rule3, rule4, rule5, rule6])
# Membuat sistem kontrol fuzzy dengan semua aturan 
stok_simulation = ctrl.ControlSystemSimulation(stok_ctrl)
# Membuat simulasi untuk sistem kontrol fuzzy yang telah dibuat
...
print("--------------------------------------")
print(f"Jumlah Stok Makanan yang Direkomendasikan: {hasil_stok:.2f} unit") # ditampilkan dalam bentuk angka dengan 2 desimal di belakang koma
...
# Menampilkan grafik hasil fuzzy
